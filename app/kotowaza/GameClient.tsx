...
import {
  getRank,
  kotowazaQuestions,
  type KotowazaQuestion,
  type KotowazaRank,
} from "@/lib/kotowaza/ranks";
...
function pickQuestions(count: number): KotowazaQuestion[] {
  return shuffle(kotowazaQuestions).slice(0, count);
}
...
/* 巻物風装飾 */
function ScrollDecoration({ position }: { position: "top" | "bottom" }) {
  const isTop = position === "top";
  return (
    <div
      className="w-full h-6 flex items-center justify-center"
      style={{
        background: "linear-gradient(180deg, #8B7355 0%, #A68B5B 50%, #8B7355 100%)",
        borderTop: isTop ? "none" : "3px solid #6B5344",
        borderBottom: isTop ? "3px solid #6B5344" : "none",
        boxShadow: isTop
...
export default function GameClient() {
  ...
  const [questions, setQuestions] = useState<KotowazaQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [shuffledChoices, setShuffledChoices] = useState<string[]>([]);
  const [correctCount, setCorrectCount] = useState(0);
  const [answerState, setAnswerState] = useState<AnswerState>("none");
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [rank, setRank] = useState<KotowazaRank | null>(null);
  ...
  const setupQuestion = useCallback((qs: KotowazaQuestion[], index: number) => {
    if (index < qs.length) {
  ...
  const goToNext = useCallback(
    (qs: KotowazaQuestion[], idx: number, correct: number) => {
      const nextIdx = idx + 1;
  ...
  const handleAnswer = useCallback(
    (choice: string) => {
      ...
      const isCorrect = choice === q.blank;
  ...
  const shareUrl =
    typeof window !== "undefined" && rank
      ? `${window.location.origin}/kotowaza?score=${correctCount}&rank=${rank.rank}&title=${encodeURIComponent(rank.title)}`
      : "";

  const shareText = rank
    ? `ことわざテストで${correctCount}/20正解！\nランク: ${rank.rank} ${rank.title}\n#ことわざテスト\n`
    : "";
  ...
  if (phase === "playing" && questions.length > 0) {
    ...
    return (
      ...
      >
        ...
        {/* Progress bar */}
        <div className="w-full max-w-md mt-10 mb-2">
          <div
        ...
        {/* Kotowaza display */}
        <div
          className="flex items-center justify-center my-6 sm:my-8 px-4"
        ...
        >
          <div
            style={{
              ...
              fontWeight: "bold",
              lineHeight: 1.4,
              animation: "kotowazaAppear 0.3s ease-out",
              letterSpacing: "0.05em",
              textAlign: "center",
            }}
          >
            {renderKotowaza(q.display)}
          </div>
        </div>
        ...
        {answerState !== "none" && (
          <div
            className="text-center mb-4 px-4 py-3 rounded-lg max-w-md"
            style={{
              ...
              animation: "meaningFade 0.3s ease-out",
              fontFamily: "serif",
            }}
          >
            {answerState === "wrong" && (
        ...
        <div className="w-full max-w-md grid grid-cols-2 gap-3 pb-8">
          {shuffledChoices.map((choice) => {
            ...
            let btnBorder = "2px solid rgba(44,24,16,0.2)";
            let btnColor = "#2C1810";
            ...
            return (
              <button
                ...
                className="choice-btn w-full py-5 px-4 rounded-lg text-xl sm:text-2xl font-bold text-center"
                style={{
                  fontFamily: "serif",
                  background: btnBg,
                  border: btnBorder,
                  ...
                  opacity: answerState !== "none" && choice !== q.blank && choice !== selectedAnswer ? 0.5 : 1,
                }}
              >
                {choice}
              </button>
  ...
  if (phase === "result" && rank) {
    ...
    return (
      ...
      >
        ...
        {/* Result header */}
        <div
          className="text-sm font-bold mt-8 mb-6"
          style={{
            color: "#8B7B6B",
            fontFamily: "serif",
