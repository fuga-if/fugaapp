...
import { getResultByScores, getTopTypes } from "@/lib/commu-style/results";
...
const typeLabels: Record<CommuStyleType, string> = {
  'type-a': '共感マスター',
  'type-b': 'ムードメーカー',
  'type-c': '的確アドバイザー',
  'type-d': '気配り調整役',
...
export function CommuStyleResultContent(): React.ReactElement {
  const router = useRouter();
  const relatedPosts = getRelatedBlogPosts("commu-style");
  ...
  const handleShare = async () => {
    const text = `【コミュスタイル診断】\n私は「${result.title}」タイプでした！\n${result.quote}\n\n#コミュスタイル診断 #fugaapp`;
    const url = `https://fugaapp.site/commu-style/result?${new URLSearchParams(Object.entries(scores).map(([k, v]) => [k, v.toString()])).toString()}`;
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg text-center animate-bounce-in">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-amber-100">
          ...
          <p className="text-amber-500 text-sm font-medium mb-2">あなたのタイプは...</p>
          ...
          <div className="bg-amber-50 rounded-2xl p-5 mb-6 text-left border border-amber-100">
            <p className="text-amber-500 font-bold text-sm mb-3 text-center">💬 このタイプの特徴</p>
          ...
          <div className="bg-gradient-to-r from-amber-100 to-pink-100 rounded-2xl p-4 mb-6 border border-amber-200">
            <p className="text-gray-800 text-lg font-bold">{result.quote}</p>
          </div>

          <div className="mb-8">
            <p className="text-amber-500 text-sm mb-3">あなたのコミュニケーション傾向</p>
              ...
              {topTypes.map((item, i) => (
                <div key={item.type} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 bg-amber-100 rounded-full h-4 overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-amber-500 to-pink-500 rounded-full" style={{ width: `${Math.min(100, (item.score / 30) * 100)}%` }} />
                  </div>
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => router.push("/commu-style")} className="w-full px-8 py-4 rounded-full font-bold text-lg shadow-lg bg-white text-amber-500 border-2 border-amber-200 hover:bg-amber-50 transition-all hover:scale-105 active:scale-95">
              もう一度診断する
            </button>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-amber-100">
            ...
            <div className="flex flex-col gap-2">
              <Link href="/shikou-type" className="px-6 py-3 bg-amber-50 hover:bg-amber-100 rounded-full text-blue-500 font-medium transition-colors text-center">🧠 思考タイプ診断 →</Link>
              <Link href="/inkya-youkya" className="px-6 py-3 bg-amber-50 hover:bg-amber-100 rounded-full text-purple-500 font-medium transition-colors text-center">🌓 陰キャ陽キャスペクトラム診断 →</Link>
              <Link href="/menhera" className="px-6 py-3 bg-amber-50 hover:bg-amber-100 rounded-full text-pink-500 font-medium transition-colors text-center">🖤 メンヘラ度診断 →</Link>
            </div>
          </div>
        </div>
      </div>
