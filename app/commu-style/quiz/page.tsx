...
import { questions, CommuStyleType } from "@/lib/commu-style/questions";

type Scores = Record<CommuStyleType, number>;
...
export default function QuizPage(): React.ReactElement {
  ...
  function handleAnswer(optionIndex: number): void {
    ...
    for (const [type, score] of Object.entries(option.scores)) {
      newScores[type as CommuStyleType] += score;
    }
    if (currentIndex >= questions.length - 1) {
      ...
      router.push(`/commu-style/result?${params.toString()}`);
      return;
    }
    setScores(newScores);
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg relative">
        {currentIndex > 0 && (
          <button onClick={handleBack} className="absolute -top-8 left-0 text-amber-500 hover:text-amber-700 text-sm flex items-center gap-1">
        ...
        <div className="w-full max-w-md mx-auto mb-8">
          ...
          <div className="w-full h-3 bg-amber-100 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-amber-500 to-pink-500 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }} />
          </div>
        </div>
            ...
            <div className="space-y-3">
              {q.options.map((option, index) => (
                <button
                  ...
                  className="w-full p-4 text-left bg-gradient-to-r from-amber-50 to-pink-50 hover:from-amber-100 hover:to-pink-100 rounded-2xl border-2 border-amber-100 hover:border-amber-400 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] group"
                >
                  <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-amber-500 to-pink-500 text-white text-sm font-bold mr-3">
