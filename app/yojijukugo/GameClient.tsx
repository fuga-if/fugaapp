...
function pickQuestions(count: number): YojijukugoQuestion[] {
  return shuffle(yojijukugoQuestions).slice(0, count);
}
...
export default function GameClient() {
  ...
  const [questions, setQuestions] = useState<YojijukugoQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [shuffledChoices, setShuffledChoices] = useState<string[]>([]);
  const [correctCount, setCorrectCount] = useState(0);
  const [answerState, setAnswerState] = useState<AnswerState>("none");
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [rank, setRank] = useState<YojijukugoRank | null>(null);
  ...
  const setupQuestion = useCallback((qs: YojijukugoQuestion[], index: number) => {
    if (index < qs.length) {
  ...
  const goToNext = useCallback(
    (qs: YojijukugoQuestion[], idx: number, correct: number) => {
      const nextIdx = idx + 1;
  ...
  const shareUrl =
    typeof window !== "undefined" && rank
      ? `${window.location.origin}/yojijukugo?score=${correctCount}&rank=${rank.rank}&title=${encodeURIComponent(rank.title)}`
      : "";

  const shareText = rank
    ? `四字熟語テストで${correctCount}/20正解！\nランク: ${rank.rank} ${rank.title}\n#四字熟語テスト\n`
    : "";
          ...
          {shuffledChoices.map((choice) => {
            ...
            return (
              <button
                ...
                className="choice-btn w-full py-5 px-4 rounded-lg text-3xl font-bold text-center"
                style={{
                  fontFamily: "serif",
                  background: btnBg,
                  border: btnBorder,
