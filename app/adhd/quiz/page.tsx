...
export default function QuizPage(): React.ReactElement {
  ...
  function handleAnswer(optionIndex: number): void {
    ...
    if (currentIndex >= questions.length - 1) {
      router.push(`/adhd/result?score=${newScore}`);
      return;
    }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg relative">
        {currentIndex > 0 && (
          <button onClick={handleBack} className="absolute -top-8 left-0 text-orange-500 hover:text-orange-700 text-sm flex items-center gap-1">
        ...
        <div className="w-full max-w-md mx-auto mb-8">
          ...
          <div className="w-full h-3 bg-orange-100 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-orange-400 to-yellow-400 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }} />
          </div>
        </div>
            ...
            <div className="space-y-3">
              {q.options.map((option, index) => (
                <button
                  ...
                  className="w-full p-4 text-left bg-gradient-to-r from-orange-50 to-yellow-50 hover:from-orange-100 hover:to-yellow-100 rounded-2xl border-2 border-orange-100 hover:border-orange-400 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] group"
                >
                  <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-orange-400 to-yellow-400 text-white text-sm font-bold mr-3">
