...
import { getResultByScores, getTopTypes } from "@/lib/neko-inu/results";
...
const typeLabels: Record<NekoInuType, string> = { 'type-a': '完全猫', 'type-b': '猫寄り猫犬', 'type-c': 'バランス型', 'type-d': '犬寄り猫犬', 'type-e': '完全犬', 'type-f': '野良猫' };

export function NekoInuResultContent(): React.ReactElement {
  const router = useRouter();
  const relatedPosts = getRelatedBlogPosts("neko-inu");
  ...
  const handleShare = async () => {
    const text = `【猫派犬派深層診断】\n私は「${result.title}」でした！\n${result.quote}\n\n#猫派犬派診断 #fugaapp`;
    const url = `https://fugaapp.site/neko-inu/result?${new URLSearchParams(Object.entries(scores).map(([k,v])=>[k,v.toString()])).toString()}`;
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg text-center animate-bounce-in">
        <div className="bg-white/70 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-pink-200">
          <div className="mb-6"><Image src={result.image} priority alt={result.title} width={180} height={180} className="mx-auto drop-shadow-[0_0_20px_rgba(255,105,180,0.3)]" /></div>
          <p className="text-pink-400 text-sm font-medium mb-2">あなたの深層タイプは...</p>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-2">{topTypes.map((item, i) => (
              <div key={item.type} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-pink-100 rounded-full h-4 overflow-hidden"><div className="h-full bg-gradient-to-r from-pink-400 to-blue-300 rounded-full" style={{ width: `${Math.min(100, (item.score / 30) * 100)}%` }} /></div>
                <span className="text-gray-600 text-xs w-20 text-left">{typeLabels[item.type]}</span>
              </div>
            ))}</div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => router.push("/neko-inu")} className="w-full px-8 py-4 rounded-full font-bold text-lg shadow-lg bg-white/50 text-pink-500 border-2 border-pink-300 hover:bg-white/70 transition-all hover:scale-105 active:scale-95">もう一度診断する</button>
          </div>
