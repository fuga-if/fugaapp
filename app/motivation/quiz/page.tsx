...
import { questions, MotivationType } from "@/lib/motivation/questions";

type Scores = Record<MotivationType, number>;
...
export default function QuizPage(): React.ReactElement {
  ...
  function handleAnswer(optionIndex: number): void {
    ...
    for (const [type, score] of Object.entries(option.scores)) {
      newScores[type as MotivationType] += score;
    }
    if (currentIndex >= questions.length - 1) {
      ...
      router.push(`/motivation/result?${params.toString()}`);
      return;
    }
    setScores(newScores);
