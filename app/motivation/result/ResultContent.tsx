...
import { getResultByScores, getTopTypes } from "@/lib/motivation/results";
...
export function MotivationResultContent(): React.ReactElement {
  const router = useRouter();
  const relatedPosts = getRelatedBlogPosts("motivation");
  ...
  const handleShare = async () => {
    const text = `【モチベーション源泉診断】\n私は「${result.title}」タイプでした！\n${result.quote}\n\n#モチベーション源泉診断 #fugaapp`;
    const url = `https://fugaapp.site/motivation/result?${new URLSearchParams(Object.entries(scores).map(([k, v]) => [k, v.toString()])).toString()}`;
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg text-center animate-bounce-in">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-purple-100">
          ...
          <div className="mb-8">
            <p className="text-purple-500 text-sm mb-3">あなたのモチベーション傾向</p>
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => router.push("/motivation")} className="w-full px-8 py-4 rounded-full font-bold text-lg shadow-lg bg-white text-purple-500 border-2 border-purple-200 hover:bg-purple-50 transition-all hover:scale-105 active:scale-95">
              もう一度診断する
            </button>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-purple-100">
            ...
            <div className="flex flex-col gap-2">
              <Link href="/commu-style" className="px-6 py-3 bg-purple-50 hover:bg-purple-100 rounded-full text-amber-500 font-medium transition-colors text-center">💬 コミュニケーションスタイル診断 →</Link>
              <Link href="/shikou-type" className="px-6 py-3 bg-purple-50 hover:bg-purple-100 rounded-full text-blue-500 font-medium transition-colors text-center">🧠 思考タイプ診断 →</Link>
              <Link href="/inkya-youkya" className="px-6 py-3 bg-purple-50 hover:bg-purple-100 rounded-full text-purple-500 font-medium transition-colors text-center">🌓 陰キャ陽キャスペクトラム診断 →</Link>
            </div>
          </div>
        </div>
      </div>
