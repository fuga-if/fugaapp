...
import { getResultByScores } from "@/lib/shikou-type/results";
import { ShikouType } from "@/lib/shikou-type/questions";
import { ShikouTypeResultContent } from "./ResultContent";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const ogUrl = `/api/og/shikou-type?type=${encodeURIComponent(result.type)}&title=${encodeURIComponent(result.title)}`;
  return {
    title: `${result.title} | 思考タイプ診断`,
...
export default function ResultPage(): React.ReactElement {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-blue-400">診断中...</div></div>}>
      <ShikouTypeResultContent />
    </Suspense>
  );
}
