...
import { SainouResultContent } from "./ResultContent";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const ogUrl = `/api/og/sainou?type=${encodeURIComponent(result.type)}&title=${encodeURIComponent(result.title)}`;
  return {
    title: `${result.title} | 隠れ才能診断`,
    description: result.description,
    openGraph: {
      title: `私の隠れた才能は「${result.title}」でした！`,
...
export default function ResultPage(): React.ReactElement {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-amber-500">才能を分析しています...</div></div>}>
      <SainouResultContent />
    </Suspense>
  );
}
