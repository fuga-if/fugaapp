...
import { questions, SainouType } from "@/lib/sainou/questions";

type Scores = Record<SainouType, number>;
...
export default function QuizPage(): React.ReactElement {
  ...
  function handleAnswer(optionIndex: number): void {
    ...
    for (const [type, score] of Object.entries(option.scores)) {
      newScores[type as SainouType] += score;
    }
    if (currentIndex >= questions.length - 1) {
      ...
      router.push(`/sainou/result?${params.toString()}`);
      return;
    }
    setScores(newScores);
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg relative">
        {currentIndex > 0 && (
          <button onClick={handleBack} className="absolute -top-8 left-0 text-amber-500/70 hover:text-amber-500 text-sm flex items-center gap-1">
            ...
            <div className="space-y-3">
              {q.options.map((option, index) => (
                <button
                  ...
                  className="w-full p-4 text-left bg-white/60 hover:bg-amber-50 rounded-xl border-2 border-amber-100 hover:border-amber-400 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] group"
                >
